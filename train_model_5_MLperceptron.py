...
# This file fits the data with a multi-layer perceptron (a dense neural network) 
...
c_here = 'c' # color in plots below

# Fit model
layers = np.array([64, 64, 64])
model = neural_network.MLPRegressor(layers, activation='relu', solver='adam', batch_size='auto', learning_rate_init=0.01, shuffle=True, early_stopping=True, n_iter_no_change=10, verbose=False)
...
# Save model
pickle.dump(model, open('model_store/model_5_MLperceptron.pickle', 'wb'))
...
plt.title(r'Training performance (multi-layer perceptron)', fontsize = title_font+4)
...
plt.title(r'Validation performance (multi-layer perceptron)', fontsize = title_font+4)
...
fig0.savefig('fig_store/fig_model_5_MLperceptron.png')
