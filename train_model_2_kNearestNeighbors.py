...
# This file fits the data with a K-nearest neighbors regressor
...
c_here = 'g' # color in plots below

# Fit model
knn = 11
model = neighbors.KNeighborsRegressor(n_neighbors=knn)
...
# Save model
pickle.dump(model, open('model_store/model_2_kNearestNeighbors.pickle', 'wb'))
...
plt.title('Training performance ('+str(knn)+' nearest neighbors)', fontsize = title_font+4)
...
plt.title('Validation performance ('+str(knn)+' nearest neighbors)', fontsize = title_font+4)
...
fig0.savefig('fig_store/fig_model_2_kNearestNeighbors.png')
