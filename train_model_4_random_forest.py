...
# This file fits the data with a random forest regressor 
...
c_here = 'darkorange' # color in plots below

# Fit model
n_trees = 50
model = ensemble.RandomForestRegressor(n_estimators=n_trees)
...
# Save model
pickle.dump(model, open('model_store/model_4_random_forest.pickle', 'wb'))
...
plt.title(r'Training performance (random forest,  $N_{\rm trees} = '+str(n_trees)+'$)', fontsize = title_font+4)
...
plt.title(r'Validation performance (random forest,  $N_{\rm trees} = '+str(n_trees)+'$)', fontsize = title_font+4)
...
fig0.savefig('fig_store/fig_model_4_random_forest.png')
